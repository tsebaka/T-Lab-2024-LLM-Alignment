...
**Все ноутбуки можно запустить на kaggle (на версиях библиотек каггловских всё окей работает) (только после reward-train, нужно сохранить reward модельку и всё)**
```
run sh run.sh # to make all scenarios reward + sft
```
```
run notebooks/resutls.ipynb # тут будут результаты с reward и вообще как моделька генерит
```
